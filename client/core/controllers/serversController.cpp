...
bool ServersController::renameServer(const QString &serverId, const QString &name)
{
    ...
    // Renaming a subscription server would mean writing its config back, and the
    // application no longer has a writer for that format.
...
QVector<ServerDescription> ServersController::buildServerDescriptions(bool isAmneziaDnsEnabled) const
{
    ...
    for (const QString &id : ids) {
        ...
        case Kind::UnsupportedSubscription:
            // Stays in the list on purpose - see serverConfigUtils.h. Connecting
            // is refused later, with an explanation.
            d = buildUnsupportedSubscriptionDescription(m_serversRepository->rawServerJson(id));
            break;
        case Kind::Invalid:
        default:
            continue;
...
QString ServersController::notificationDisplayName(const QString &serverId) const
{
    ...
    case Kind::UnsupportedSubscription: {
        const QJsonObject stored = m_serversRepository->rawServerJson(serverId);
...
bool ServersController::isUnsupportedSubscriptionServer(const QString &serverId) const
{
    return !serverId.isEmpty()
            && serverConfigUtils::isUnsupportedSubscription(m_serversRepository->serverKind(serverId));
}
