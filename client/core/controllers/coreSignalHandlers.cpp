...
{
#if !defined(Q_OS_ANDROID) && !defined(Q_OS_IOS)
    // The notification used to be filed in the news list as well, so a missed
    // drawer could be reopened from there. The news list is gone along with the
    // subscription layer, so the drawer is now the only chance to see it.
