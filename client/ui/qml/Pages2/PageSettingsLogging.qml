...
PageType {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("AbstractumVPN logs")
        ...
        readonly property var exportLogsHandler: function() {
            var fileName = ""
            if (GC.isMobile()) {
                fileName = "AbstractumVPN.log"
            } else {
                fileName = SystemController.getFileName(qsTr("Save"),
                                                        qsTr("Logs files (*.log)"),
                                                        StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/AbstractumVPN",
                                                        true,
                                                        ".log")
            }
            if (fileName !== "") {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("AbstractumVPN-service logs")
        ...
        readonly property var exportLogsHandler: function() {
            ...
            fileName = SystemController.getFileName(qsTr("Save"),
                                                    qsTr("Logs files (*.log)"),
                                                    StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/AbstractumVPN-service",
                                                    true,
                                                    ".log")
            if (fileName !== "") {
                PageController.showBusyIndicator(true)
